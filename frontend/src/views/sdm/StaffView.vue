...
async function bulkDelete() {
  ...
  if (!confirm(`Hapus ${selectedIds.value.length} tenaga kependidikan yang dipilih?`)) return
  bulkDeleting.value = true
  try { const { data } = await api.post('/staff/bulk-delete', { ids: selectedIds.value }); toast.success(data.message); selectedIds.value = []; load() }
