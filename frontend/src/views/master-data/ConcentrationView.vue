...
interface Concentration {
  id: number; code: string; name: string; status: boolean; study_program?: StudyProgram
}

const { items, pagination, loading, fetchAll, create, update, remove } = useCrud<Concentration>('/concentrations')
const programs = ref<StudyProgram[]>([])
const search = ref(''); const filterProgram = ref('')
const modalOpen = ref(false); const editingId = ref<number | null>(null); const saving = ref(false)
const form = reactive({ study_program_id: '', code: '', name: '', description: '', status: true })

const columns = [
  { key: 'code', label: 'Kode' }, { key: 'name', label: 'Nama Konsentrasi' },
  { key: 'program', label: 'Program Studi' }, { key: 'status', label: 'Status' },
...
async function load(page = 1) { await fetchAll({ search: search.value, study_program_id: filterProgram.value, page }) }

function openCreate() {
  editingId.value = null
  Object.assign(form, { study_program_id: '', code: '', name: '', description: '', status: true })
  modalOpen.value = true
}

function openEdit(item: Concentration) {
  editingId.value = item.id
  Object.assign(form, { study_program_id: item.study_program?.id ?? '', code: item.code, name: item.name, status: item.status })
  modalOpen.value = true
}
...
async function handleDelete(item: Concentration) {
  if (!confirm(`Hapus konsentrasi "${item.name}"?`)) return
...
<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold text-gray-900">Konsentrasi</h1>
        <p class="text-sm text-gray-500 mt-0.5">Kelola peminatan / konsentrasi per program studi</p>
      </div>
      <button class="flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors" @click="openCreate">
        <PlusIcon class="w-4 h-4" /> Tambah Konsentrasi
      </button>
    </div>
    <div class="flex flex-wrap gap-3">
      <input v-model="search" type="text" placeholder="Cari nama..." class="px-3.5 py-2 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-56" @input="load()" />
  ...
  <BaseModal :open="modalOpen" :title="editingId ? 'Edit Konsentrasi' : 'Tambah Konsentrasi'" @close="modalOpen = false">
    <form class="space-y-4" @submit.prevent="handleSave">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Nama Konsentrasi <span class="text-red-500">*</span></label>
